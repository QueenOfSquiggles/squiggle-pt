...
namespace queen
{
    ...
    public partial class HelperPlugin : EditorPlugin
    {
        ...
        public static void ToolEditConfiguration()
        {
            _ = OS.ShellOpen(ProjectSettings.GlobalizePath(CONFIG_RELEASE));
        }

        public static void ToolReleaseBuilds()
        {
            string script = ProjectSettings.GlobalizePath(PY_RELEASE);
            Array output = new();
            RunTerminalCommand(script.GetBaseDir(), script, System.Array.Empty<string>());
            PrintOutput(output);
        }

        public static void ToolCleanBuilds()
        {
            string script = ProjectSettings.GlobalizePath(PY_RELEASE);
            RunTerminalCommand(script.GetBaseDir(), script, new string[] { "./export", "--clean" });
        }

        private static void RunTerminalCommand(string dir, string py_script, string[] args)
        {
            Array output = new();
            ...
            _ = OS.Execute("/usr/bin/konsole", cmd_buffer, output, true);
            PrintOutput(output);
        }

        private static void PrintOutput(Array lines)
        {
            Print.MsgOut(new Msg("--- Command Output ---").AlignCenter().Color("pink"));
            foreach (Variant l in lines)
            {
